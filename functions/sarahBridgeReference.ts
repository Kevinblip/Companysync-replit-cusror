/**
 * ============================================================
 * REFERENCE ONLY — Deploy this code to Railway, NOT to Base44.
 * ============================================================
 * 
 * Sarah Voice Bridge — Twilio ↔ Gemini Live API
 * 
 * This is a WebSocket server that bridges:
 *   Twilio Media Streams (mulaw 8kHz) ↔ Gemini Live API (PCM 16kHz in / 24kHz out)
 * 
 * DEPLOYMENT:
 *   1. Create a new Railway project (Node.js)
 *   2. Push this as server.js  (entry point)
 *   3. Set env vars: GOOGLE_GEMINI_API_KEY, SARAH_BRIDGE_SECRET
 ...
 *   - SARAH_BRIDGE_SECRET    (shared secret – must match Base44 secret)
 * ============================================================
 */
...
// Use the correct model name for native audio dialog.
// Check https://ai.google.dev/gemini-api/docs/models for current names.
// As of early 2025, working options include:
...
// ╔══════════════════════════════════════════════════════════╗
// ║  AUDIO CONVERSION — ITU G.711 µ-law  ↔  Linear PCM     ║
// ╚══════════════════════════════════════════════════════════╝

// Standard ITU G.711 µ-law decode table (256 entries → Int16)
const MULAW_DECODE_TABLE = new Int16Array(256);
(function buildDecodeTable() {
  for (let i = 0; i < 256; i++) {
    let val = ~i & 0xFF;                       // complement
    const sign = val & 0x80;                   // sign bit
    const exponent = (val >> 4) & 0x07;        // 3-bit exponent
    const mantissa = val & 0x0F;               // 4-bit mantissa
    let magnitude = ((mantissa << 1) + 33) << (exponent + 2);  // shift up
    magnitude -= 33 * 16;                      // remove bias
    MULAW_DECODE_TABLE[i] = sign ? -magnitude : magnitude;
  }
})();

// Standard ITU G.711 µ-law encode (Int16 → Uint8)
function mulawEncode(sample) {
  const CLIP  = 32635;
  const BIAS  = 0x84;   // 132 = 33 << 2
...
/**
 * Twilio mulaw-8kHz (base64) → Gemini PCM-16kHz (base64)
 *   1. µ-law decode → PCM 8 kHz
 *   2. Linear-interpolation upsample 8 → 16 kHz
 */
function twilioToGemini(mulawB64) {
  const src  = Buffer.from(mulawB64, 'base64');
  const nSrc = src.length;

  // 1. Decode µ-law → 16-bit PCM @ 8 kHz
  const pcm8k = new Int16Array(nSrc);
  for (let i = 0; i < nSrc; i++) {
    pcm8k[i] = MULAW_DECODE_TABLE[src[i]];
  }

  // 2. Upsample 8 → 16 kHz (linear interp)
  const nDst  = nSrc * 2;
...
/**
 * Gemini PCM-24kHz (base64) → Twilio mulaw-8kHz (base64)
 *   1. Parse LE-16 PCM samples
 *   2. Downsample 24 → 8 kHz  (take every 3rd sample)
 *   3. µ-law encode
 */
function geminiToTwilio(pcmB64) {
  const buf  = Buffer.from(pcmB64, 'base64');
  const nSrc = buf.length >> 1;                // 16-bit samples

  // Downsample 24 → 8 kHz  (factor = 3)
  const nDst = Math.floor(nSrc / 3);
  const out  = Buffer.alloc(nDst);
  for (let i = 0; i < nDst; i++) {
    const sample = buf.readInt16LE(i * 6);     // every 3rd sample, 2 bytes each
...
wss.on('connection', (twilioWs) => {
  ...
  let params       = {};
  const audioQueue = [];           // buffer while Gemini isn't ready yet
    ...
    switch (msg.event) {
      ...
      case 'media':
        if (geminiReady && geminiWs?.readyState === WebSocket.OPEN) {
          ...
          if (audioQueue.length > 200) audioQueue.shift();   // keep bounded
        }
        break;

      case 'mark':
        // Twilio finished playing the chunk we sent
        break;
  ...
  function openGemini(p) {
    ...
    geminiWs.on('open', () => {
      ...
      // ── SETUP (must be the very first message) ──
      geminiWs.send(JSON.stringify({
        setup: {
    ...
    geminiWs.on('message', (raw) => {
      ...
      if (msg.setupComplete) {
        ...
        // ── Keep-alive: send silence every 15s so Gemini doesn't timeout ──
        keepAliveTimer = setInterval(() => {
          if (!geminiReady || geminiWs?.readyState !== WebSocket.OPEN) return;
          // 160 samples of silence at 16 kHz = 10 ms
          const silence = Buffer.alloc(320); // 160 samples × 2 bytes = 320
          geminiWs.send(JSON.stringify({
            realtimeInput: {
      ...
      if (msg.serverContent?.turnComplete) {
        ...
        // Send a mark so we know when Twilio finishes playback
