...
Deno.serve(async (req) => {
    try {
        ...
        console.log(`[V6] Starting enhanced analysis for: ${address}`);

        // Try Google Geocoding first
        try {
            console.log('[V6] Attempting Google Geocoding...');
            ...
            if (geoData.status === 'OK' && geoData.results[0]) {
                ...
                console.log(`[V6] Google Geocoding success: ${lat}, ${lng}`);
            } else {
                console.warn(`[V6] Google Geocoding failed: ${geoData.status}`);
            }
        } catch (e) {
            console.warn(`[V6] Google Geocoding exception: ${e.message}`);
        }

        // Fallback to Nominatim if Google failed
        if (!lat || !lng) {
            console.log('[V6] Falling back to Nominatim for geocoding...');
            try {
                ...
                if (nominatimResp.ok) {
                    ...
                    if (nominatimData && nominatimData.length > 0) {
                        ...
                        console.log(`[V6] Nominatim success: ${lat}, ${lng}`);
                    } else {
                        console.warn('[V6] Nominatim returned no results');
                    }
                } else {
                    console.warn(`[V6] Nominatim failed: ${nominatimResp.status}`);
                }
            } catch (e) {
                console.error(`[V6] Nominatim Geocoding error: ${e.message}`);
            }
        }
        ...
        console.log(`[V6] Fetching static map: ${staticMapUrl}`);
        
        let satellite_image_url = null;
        let mapFetchSuccess = false;
        
        try {
            const mapResp = await fetch(staticMapUrl);
            if (mapResp.ok) {
                const mapBlob = await mapResp.blob();
                console.log(`[V6] Downloaded map blob type: ${mapBlob.type}, size: ${mapBlob.size}`);
                
                if (mapBlob.type.includes("image")) {
                    const mapFile = new File([mapBlob], "satellite_view.png", { type: "image/png" });
                    
                    console.log('[V6] Uploading satellite image to storage...');
                    try {
                        ...
                        if (uploadRes && uploadRes.file_url) {
                            ...
                            mapFetchSuccess = true;
                            console.log(`[V6] Uploaded satellite URL: ${satellite_image_url}`);
                        } else {
                            console.warn('[V6] UploadFile returned no file_url');
                        }
                    } catch (uploadErr) {
                         console.error('[V6] UploadFile integration failed:', uploadErr);
                    }
                } else {
                     console.warn(`[V6] Downloaded blob is not an image: ${mapBlob.type}`);
                }
            } else {
                console.warn(`[V6] Static Map API fetch failed: ${mapResp.status} ${mapResp.statusText}`);
            }
        } catch (e) {
            console.error('[V6] Error fetching/uploading static map:', e);
        }

        // 3. Analyze with AI - Enhanced prompt for measurement indicators
        let analysis = {
            ai_description: "Satellite imagery unavailable. Analysis skipped.",
            roof_type: "unknown",
            roof_complexity: "unknown",
            total_roof_planes: 0,
            building_footprint_shape: "unknown",
            linear_features: { ridge_count: 0, hip_count: 0, valley_count: 0, rake_count: 0, eave_count: 0 },
            structural_features: { has_garage: false, has_additions: false, dormer_count: 0, chimney_count: 0 },
            sections_identified: [],
            measurement_hints: { symmetry: "unknown", dominant_pitch_category: "unknown", estimated_stories: 1 }
        };

        if (mapFetchSuccess && satellite_image_url) {
            console.log('[V6] Invoking LLM for enhanced analysis...');
            try {
...
Provide structured, actionable data that can improve automated roof measurement calculations.`,
                ...
                console.log('[V6] LLM Analysis complete');
            } catch (llmError) {
                console.error('[V6] LLM Analysis failed:', llmError);
                analysis.ai_description = "AI analysis failed during processing. Please try again.";
            }
        } else {
            console.warn('[V6] Skipping LLM analysis due to missing map image');
            analysis.ai_description = "Satellite image could not be retrieved (likely due to API key restrictions). Location was found successfully.";
        }

        return Response.json({
            success: true,
            satellite_image_url: satellite_image_url || null, 
            roof_layout_analysis: analysis,
            location: { lat, lng }
        });

    } catch (error) {
        console.error('analyzeSatelliteImageNew critical error:', error);
