...
export default function AccountsReceivableReport() {
  ...
  const { data: invoices = [], isLoading } = useQuery({
    queryKey: ["ar-invoices", myCompany?.id],
    queryFn: () => myCompany ? base44.entities.Invoice.filter({ company_id: myCompany.id }, "-issue_date", 1000) : [],
    initialData: [],
    enabled: !!myCompany,
  });
                  ...
                  {outstandingInvoices.map((invoice) => {
                    ...
                    return (
                      <TableRow key={invoice.id}>
                        ...
                        <TableCell className="text-right">${Number(invoice.amount || 0).toFixed(2)}</TableCell>
