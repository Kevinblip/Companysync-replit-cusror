...
export default function Utilities() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {(myStaffProfile?.is_super_admin || myCompany?.created_by === user?.email || myCompany?.id === 'companysync_master_001') && (
            ...
            <Button
              onClick={async () => {
                if (confirm('Recalculate all invoice payment totals? This will fix any out-of-sync invoices.')) {
                  try {
                    const allPayments = myCompany ? await base44.entities.Payment.filter({ company_id: myCompany.id }, '-created_date', 10000) : [];
                  ...
                  <div className="space-y-2">
                    {companies.map(c => (
                      <div key={c.id} className="flex items-center justify-between p-2 bg-white rounded border border-red-200">
                        ...
                        {c.id !== 'companysync_master_001' && !c.company_name?.includes('Insurance Claims Network') && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={async () => {
          ...
          <CardContent className="space-y-4">
            <div className="text-sm text-gray-700">
              ...
              <ul className="list-disc list-inside space-y-1 text-gray-600">
                ...
                <li><strong>Protects:</strong> CompanySync and all valid active companies</li>
