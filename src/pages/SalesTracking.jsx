...
export default function SalesTracking() {
  ...
  const { data: invoices = [] } = useQuery({
    ...
    queryFn: () => myCompany ? base44.entities.Invoice.filter({ company_id: myCompany.id }, "-created_date") : [],
    enabled: !!myCompany,
    initialData: [],
  });
  ...
  const totalValue = filteredLeads.reduce((sum, l) => sum + Number(l.value || 0), 0);
  const wonLeads = filteredLeads.filter(l => l.status === 'won').length;
  const conversionRate = totalLeads > 0 ? ((wonLeads / totalLeads) * 100).toFixed(1) : 0;
  const avgDealSize = wonLeads > 0 ? (filteredLeads.filter(l => l.status === 'won').reduce((sum, l) => sum + Number(l.value || 0), 0) / wonLeads).toFixed(0) : 0;
  ...
  const teamPerformance = allStaffProfiles.map(staff => {
    ...
    const staffRevenue = invoices.filter(i => i.sale_agent === staff.user_email && i.status === 'paid').reduce((sum, i) => sum + Number(i.amount || 0), 0);
