...
export default function GeminiLiveClient({ systemPrompt: propPrompt, model: propModel, companyId }) {
        ...
        const [sessionId, setSessionId] = React.useState(() => `voice_session_${Date.now()}`);
          ...
          const currentSourceRef = React.useRef(null); // Store current audio source to stop it
...
- Be warm, professional, and helpful.`;
  ...
  // BUG FIX: Interruption & duplicate prevention refs
  ...
  // BUG FIX: Cancel ongoing LLM request
  ...
  const playNextChunk = async () => {
    ...
    // CRITICAL: Force resume AudioContext before every playback
    ...
    try {
      ...
      // Create buffer (Gemini: 24kHz mono)
      ...
      // Boost volume slightly for speech clarity
  ...
  const startSession = async () => {
         ...
         setStatus("connecting");
          ...
          console.log('📢 Skipping browser TTS greeting - Gemini will handle audio');
      ...
      ws.onmessage = async (event) => {
        ...
        try {
            ...
            if (event.data instanceof Blob) {
                ...
                if (!text || text.trim() === '') {
                    console.warn('⚠️ Empty blob received, ignoring');
                    return;
                }
                try {
                    data = JSON.parse(text);
                } catch (parseErr) {
                    console.error('❌ JSON parse error on blob:', text.substring(0, 100));
                    return;
                }
            } else {
                if (!event.data || typeof event.data !== 'string' || event.data.trim() === '') {
                    console.warn('⚠️ Empty or invalid message received, ignoring');
                    return;
                }
                try {
                    data = JSON.parse(event.data);
                } catch (parseErr) {
                    console.error('❌ JSON parse error on string:', event.data.substring(0, 100));
                    return;
                }
            }
            ...
            if (data.system_status) {
                ...
                if (data.system_status === "google_connected") {
                    ...
                    // Gemini Live API - configure with voice at setup root
                     const setupMsg = {
                         setup: {
                    ...
                    console.log("📤 SENDING SETUP - response_modalities as ARRAY:", JSON.stringify(setupMsg, null, 2));
             ...
             if (data.serverContent?.userTurn?.parts) {
                 ...
                 for (const part of data.serverContent.userTurn.parts) {
                     if (part.text) {
                         ...
                         // BUG FIX: Immediately stop any ongoing playback when user speaks
                         stopAllAudio();
                         cancelCurrentRequest();
                         
                         // Check for "disregard" command to stop talking
                         if (part.text.toLowerCase().includes('disregard') || part.text.toLowerCase().includes('never mind') || part.text.toLowerCase().includes('stop')) {
                             console.log('🛑 Stop command detected - halting');
                             setStatus("live");
                             return; // Don't process further
                         }

                         // BUG FIX: Generate unique request ID to prevent duplicate responses
                         ...
                         console.log('🎤 [Lexi Voice] Processing request:', requestId);
                         ...
                         // BUG FIX: Skip "please hold" for short queries - go straight to response
                         const isQuickQuery = part.text.split(' ').length <= 5;
                         
                         if (!isQuickQuery && wsRef.current?.readyState === WebSocket.OPEN) {
                             console.log('[Lexi Voice] 📢 Speaking acknowledgment...');
                             wsRef.current.send(JSON.stringify({
                                 client_content: {
                                     turns: [{
                                         role: "user",
                                         parts: [{ text: 'One moment.' }]
                                     }],
                                     turn_complete: true
                                 }
                             }));
                         ...
                         base44.functions.invoke('lexiChat', {
                             ...
                             // BUG FIX: Check if this request is still current (not interrupted)
                             if (currentRequestIdRef.current !== requestId) {
                                 console.log('[Lexi Voice] ⏭️ Request superseded, ignoring response for:', requestId);
                                 return;
                             }

                             console.log('[Lexi Voice] Brain response received');
                             const textResponse = result.data?.response;

                             if (!textResponse) {
                                 console.error('[Lexi Voice] ❌ Empty response');
                                 isProcessingRef.current = false;
                                 return;
                             }
                             ...
                             // BUG FIX: Final check before sending to TTS
                             if (currentRequestIdRef.current !== requestId) {
                                 console.log('[Lexi Voice] ⏭️ Request interrupted before TTS');
                                 return;
                             }

                             if (wsRef.current?.readyState === WebSocket.OPEN) {
                                 console.log('[Lexi Voice] ✅ Sending response to Gemini TTS');
                                 wsRef.current.send(JSON.stringify({
                                     client_content: {
                                         turns: [{
                                             role: "user",
                                             parts: [{ text: `Say exactly this: "${textResponse}"` }]
                                         }],
                                         turn_complete: true
                                     }
                                 }));
                             ...
                             // BUG FIX: Only show error if this request is still current
                             if (currentRequestIdRef.current !== requestId) return;
                             
                             console.error('[Lexi Voice] ❌ Brain error:', err);
                             isProcessingRef.current = false;
                             
                             // BUG FIX: Single error message, no retry spam
                             if (wsRef.current?.readyState === WebSocket.OPEN) {
                                 wsRef.current.send(JSON.stringify({
                                     client_content: {
                                         turns: [{
            ...
            if (err instanceof SyntaxError) {
                console.error("JSON Parse Error - message content:", typeof event.data === 'string' ? event.data : 'Blob');
            }
        }
      };
  ...
  const startAudioRecording = async (ws) => {
      ...
      processor.onaudioprocess = (e) => {
          if (ws.readyState === WebSocket.OPEN && micEnabledRef.current && isReadyRef.current) {
              ...
              // BUG FIX: Improved interruption logic
              if (rms > SILENCE_THRESHOLD) {
                speechFrameCount++;
                
                // Only interrupt after confirmed speech (prevents false triggers)
                if (speechFrameCount >= SPEECH_CONFIRM_FRAMES) {
                  const now = Date.now();
                  const timeSinceLastInterrupt = now - lastInterruptTimeRef.current;
                  
                  // Check if AI is speaking or has pending audio AND cooldown passed
                  if ((isPlayingRef.current || audioQueueRef.current.length > 0 || isProcessingRef.current) && timeSinceLastInterrupt > INTERRUPT_COOLDOWN) {
                    console.log('🎤 USER INTERRUPTING (RMS:', rms.toFixed(4), ') - STOPPING EVERYTHING');
                    lastInterruptTimeRef.current = now;

                    // 1. IMMEDIATELY stop all audio playback
                    stopAllAudio();

                    // 2. Cancel any pending LLM request
                    cancelCurrentRequest();

                    // 3. Send interrupt signal to Gemini to stop generating
                ...
                speechFrameCount = 0; // Reset on silence
              }

              if (isPlayingRef.current || audioQueueRef.current.length > 0 || isProcessingRef.current || Date.now() < echoGuardUntilRef.current) {
                return;
              }
      ...
      // ECHO FIX: Route processor through a silent gain node (gain=0) instead of directly
      // to destination. ScriptProcessor requires a destination connection to stay active
      // in some browsers, but connecting it directly to speakers causes mic echo.
  ...
  const stopSession = () => {
    ...
    // BUG FIX: Cancel any pending requests first
    cancelCurrentRequest();
    stopAllAudio();
