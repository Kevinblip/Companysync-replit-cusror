...
function polylinePoints(outline, width, height, pad = 18) {
  ...
  const maxU = Math.max(...outline.map(p => p.u), width, 0.01);
  const maxV = Math.max(...outline.map(p => p.v), height, 0.01);
...
export default function ElevationDrawings({ elevations = [] }) {
  return (
    <div className="space-y-2" data-testid="elevation-drawings">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {elevations.map(el => {
          const photographed = Boolean(el.photographed && el.photoUrl);
          const estimated = Boolean(el.estimated) && !photographed;
          ...
          return (
            <div
              ...
              className={`rounded-lg border bg-white p-3 ${
                photographed ? 'border-emerald-300' : estimated ? 'border-dashed border-orange-300' : 'border-slate-200'
              }`}
            >
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`text-xs font-medium ${
                    photographed ? 'text-emerald-800' : estimated ? 'text-orange-700' : 'text-gray-700'
                  }`}
                  data-testid={`elevation-${el.id}-status`}
                >
                  {el.label} — {photographed ? 'photographed' : estimated ? 'estimated' : 'from measurements'}
                </span>
              ...
              {photographed ? (
                <div className="relative rounded overflow-hidden bg-slate-100" data-testid={`elevation-${el.id}-photo`}>
                  <img
                    src={el.photoUrl}
                    alt={`${el.label} facade photo`}
                    className="w-full h-28 object-cover"
                  />
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-2 py-1.5 flex justify-between text-[10px] text-white">
                    <span>{el.height_ft} ft</span>
                    <span>{el.caption}</span>
                    <span>{el.width_ft} ft</span>
                  </div>
                </div>
              ) : (
