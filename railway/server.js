...
wss.on("connection", async (twilioWs, req) => {
  ...
  try {
    ...
    // Debug: log raw Gemini message keys for the first 30s — only when GEMINI_SCHEMA_DEBUG=true
    const schemaDebugEnabled = process.env.GEMINI_SCHEMA_DEBUG === 'true';
    const debugUntil = schemaDebugEnabled ? Date.now() + 30000 : 0;

    geminiWs.on("message", async (rawData) => {
      try {
        const data = JSON.parse(rawData.toString());

        // Schema-only debug logging — logs keys, never content
        if (schemaDebugEnabled && Date.now() < debugUntil && data.serverContent) {
          const keys = Object.keys(data.serverContent);
          if (keys.length > 0) {
            const transcriptTypes = {};
            if (data.serverContent.inputTranscript !== undefined) transcriptTypes.inputTranscript = typeof data.serverContent.inputTranscript;
            if (data.serverContent.outputTranscript !== undefined) transcriptTypes.outputTranscript = typeof data.serverContent.outputTranscript;
            console.log(`[GEMINI-SCHEMA] serverContent keys: ${keys.join(', ')}${Object.keys(transcriptTypes).length ? ' | types: ' + JSON.stringify(transcriptTypes) : ''}`);
          }
        }

        // Capture caller transcript — handle both string and {text:"..."} object formats
        if (data.serverContent?.inputTranscript) {
          const it = data.serverContent.inputTranscript;
          const callerText = typeof it === 'string' ? it : (it?.text || JSON.stringify(it));
          if (callerText) conversationLog.push({ role: 'Caller', text: callerText });
        }

        // Capture Sarah's transcript — outputTranscript for native audio model
        if (data.serverContent?.outputTranscript) {
          const ot = data.serverContent.outputTranscript;
          const sarahText = typeof ot === 'string' ? ot : (ot?.text || JSON.stringify(ot));
          if (sarahText) conversationLog.push({ role: assistantName, text: sarahText });
        }
